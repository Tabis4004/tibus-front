...
class EscPosLinesEncoder {
  ...
  static void _writeAligned(
    ...
    // GS ! n — taille : 0x00 normal, 0x01 double hauteur seule (utilisé pour
    // "medium", ex. téléphone du destinataire — pas d'équivalent "+2 pts"
    // exact sur du matériel ESC/POS thermique, seuls des multiplicateurs
    // entiers de largeur/hauteur sont disponibles ; double hauteur seule est
    // l'augmentation la plus proche d'un léger agrandissement sans doubler
    // aussi la largeur, demande explicite du 27/08/2026), 0x11 double
    // largeur+hauteur ("large").
    bytes.addByte(_gs);
    bytes.addByte(0x21);
    bytes.addByte(size == 'large' ? 0x11 : (size == 'medium' ? 0x01 : 0x00));
