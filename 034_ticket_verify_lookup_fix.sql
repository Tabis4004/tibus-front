-- Lot 34: Correctif recherche billet — lookup robuste + détails toujours renvoyés.
...
BEGIN
  ...
  WHERE rb."type" IN ('voyage', 'colisVoyage')
    AND (
      public.normalize_ticket_reference(p.reference) = v_ref
      OR UPPER(REGEXP_REPLACE(BTRIM(p.reference), '\s+', '', 'g')) = v_ref_alt
    )
  ...
  IF v_rb.id IS NULL THEN
    RETURN jsonb_build_object(
      'valid', false,
      'result', 'not_found',
      'message', 'Aucun billet actif ne correspond à cette référence',
  ...
  v_payload := jsonb_build_object(
    ...
    'trip', CASE
      WHEN v_departure_time IS NULL THEN NULL
      ELSE jsonb_build_object('departureTime', v_departure_time, 'arrivalTime', NULL)
    END,
