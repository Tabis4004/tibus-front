-- 069 — Hotfix chargement soldes stakeholders (réaligne earned_rows + pool 066)
-- À exécuter si l'admin affiche « Impossible de charger les soldes stakeholders »
-- après 065/066 (souvent : 065 ré-appliqué après 066 → base_amount 4 args manquant).
-- PRÉREQUIS : 064, 065, 066
...
AS $$
  ...
  LEFT JOIN LATERAL public.resolve_seller_commission_setting(p_company_id) AS resolved ON true
  LIMIT 1;
$$;
...
AS $$
  SELECT CASE p_base_type
    ...
    WHEN 'total_amount' THEN COALESCE(p_total_amount, p_gateway_amount, 0)
    WHEN 'platform_net' THEN COALESCE(p_platform_net_amount, p_gateway_amount, 0)
...
GRANT EXECUTE ON FUNCTION public._stakeholder_commission_base_amount(text, double precision, double precision, double precision, double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public._stakeholder_commission_earned_rows(uuid) TO authenticated;
