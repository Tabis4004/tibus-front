-- Lot 38: Confirmation montée à bord (on board) — distinct du scan embarquement.

ALTER TABLE "ReservationBus"
  ADD COLUMN IF NOT EXISTS "onBoardAt" timestamptz,
  ADD COLUMN IF NOT EXISTS "onBoardBy" uuid REFERENCES "Users" (id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS "onBoardScanCount" integer NOT NULL DEFAULT 0;
...
CREATE OR REPLACE FUNCTION public.confirm_passenger_on_board(p_reference text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  SELECT rb.id, rb."reservationId", rb."onBoardAt"
  INTO v_rb
  FROM "Payment" p
  JOIN "ReservationBus" rb ON rb."paymentId" = p.id
  WHERE UPPER(BTRIM(p.reference)) LIKE '%' || v_fragment || '%'
  ...
  IF v_rb.id IS NULL THEN
    RETURN jsonb_build_object('valid', false, 'result', 'not_found', 'message', 'Billet introuvable');
  END IF;
  ...
  IF NOT public.is_super_admin()
    ...
    RETURN jsonb_build_object('valid', false, 'result', 'wrong_company', 'message', 'Ce billet appartient à une autre compagnie');
  END IF;

  IF v_rb."onBoardAt" IS NOT NULL THEN
    RETURN public.lookup_ticket_for_verify(p_reference)
      || jsonb_build_object(
        'valid', false,
        'result', 'on_board',
        'message', 'already_on_board',
        'status', 'collected'
      );
  END IF;
  ...
  SET
    ...
    "boardedAt" = COALESCE("boardedAt", now()),
    "boardedBy" = COALESCE("boardedBy", v_user_id),
    "boardingScanCount" = CASE WHEN "boardedAt" IS NULL THEN COALESCE("boardingScanCount", 0) + 1 ELSE "boardingScanCount" END
  WHERE id = v_rb.id;
...
GRANT EXECUTE ON FUNCTION public.confirm_passenger_on_board(text) TO authenticated;
