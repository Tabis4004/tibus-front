-- =============================================================================
-- Tibus 149 — Vente guichet : autoriser vendeur_gare (rôle gare, vendeur compagnie obsolète)
-- =============================================================================

CREATE OR REPLACE FUNCTION public.can_seller_counter_sale(
  p_user_id uuid,
  p_company_id uuid
)
RETURNS boolean
...
GRANT EXECUTE ON FUNCTION public.can_seller_counter_sale(uuid, uuid) TO authenticated;

CREATE OR REPLACE FUNCTION public.seller_counter_sale(
  p_reservation_id uuid,
  p_passenger_name text,
  p_passenger_phone text DEFAULT NULL::text,
  p_seat_number text DEFAULT NULL::text,
...
BEGIN
  ...
  IF NOT public.can_seller_counter_sale(v_user_id, v_company_id) THEN
    RAISE EXCEPTION 'Vente guichet non autorisee pour ce compte';
  END IF;
