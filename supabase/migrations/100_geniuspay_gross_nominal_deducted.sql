-- 100_geniuspay_gross_nominal_deducted.sql
-- GeniusPay : montant API = T = M × (1 + X% + Y% + Z%) + F
-- GeniusPay déduit Y%×M + Z%×M + F ; net marchand = M × (1 + X%).
...
BEGIN
  ...
  IF v_is_geniuspay THEN
    ...
    v_raw_min := p_nominal_amount * (1 + (v_x + v_y_min + v_z) / 100.0) + v_f;
    v_raw_max := p_nominal_amount * (1 + (v_x + v_y_max + v_z) / 100.0) + v_f;
    v_total := CEIL(v_raw);
    v_total_min := CEIL(v_raw_min);
    v_total_max := CEIL(v_raw_max);
    v_fee_mode := 'deducted_on_nominal';
