-- 101_geniuspay_deducted_on_gross.sql
-- GeniusPay déduit Y%×T + Z%×T + F du montant brut T (pas sur M).
-- T = (V + F) / (1 - (Y% + Z%)/100)  avec V = M × (1 + X%).
...
DECLARE
  ...
  v_raw_min double precision;
  v_raw_max double precision;
  v_total double precision;
  v_total_min double precision;
  v_total_max double precision;
...
BEGIN
  ...
  IF v_is_geniuspay THEN
    ...
    IF (v_y + v_z) >= 100 OR (v_y_min + v_z) >= 100 OR (v_y_max + v_z) >= 100 THEN
      RAISE EXCEPTION 'Taux GeniusPay invalides: Y+Z >= 100%%';
    END IF;

    v_denominator := 1 - (v_y + v_z) / 100.0;
    v_raw := (v_v + v_f) / v_denominator;
    v_raw_min := (v_v + v_f) / (1 - (v_y_min + v_z) / 100.0);
    v_raw_max := (v_v + v_f) / (1 - (v_y_max + v_z) / 100.0);
    v_total := CEIL(v_raw);
    v_total_min := CEIL(v_raw_min);
    v_total_max := CEIL(v_raw_max);
    v_fee_mode := 'deducted_on_gross';
    ...
    RETURN jsonb_build_object(
      ...
      'totalAmount', v_total,
      'totalAmountMin', v_total_min,
      'totalAmountMax', v_total_max,
      'gatewayAmount', CASE WHEN v_network_known THEN v_total ELSE v_total_max END,
      'feeMode', v_fee_mode,
