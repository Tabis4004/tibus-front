-- 099_geniuspay_platform_margin_only.sql
-- GeniusPay gère Y% (réseau), Z% (1%) et F (100) sur son checkout.
-- Tibus envoie uniquement V = M × (1 + X%) ; le total voyageur varie selon le réseau.
...
BEGIN
  ...
  IF v_is_geniuspay THEN
    ...
    v_y := CASE WHEN v_fees_found THEN COALESCE(v_fees.y_percent, 0) ELSE 0 END;
    v_z := CASE WHEN v_fees_found THEN COALESCE(v_fees.z_percent, 0) ELSE 0 END;
    v_f := CASE WHEN v_fees_found THEN COALESCE(v_fees.f_fixed, 0) ELSE 0 END;
    ...
    RETURN jsonb_build_object(
      ...
      'method', lower(COALESCE(v_fees.method, p_method)),
      'network', 'geniuspay',
      'requestedNetwork', lower(COALESCE(NULLIF(trim(p_network), ''), 'unknown')),
      'usedMaxFallback', false,
