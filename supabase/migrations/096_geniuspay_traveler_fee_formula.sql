-- 096_geniuspay_traveler_fee_formula.sql
-- GeniusPay : le réseau MMO est choisi sur le checkout GeniusPay.
-- Montant voyageur Tibus = commission plateforme X + frais GeniusPay Z + fixe F (sans Y opérateur).
...
BEGIN
  ...
  v_y := CASE WHEN v_is_geniuspay THEN 0 ELSE COALESCE(v_fees.y_percent, 0) END;
