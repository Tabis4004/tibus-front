-- Colis autonomes : encaissement caisse vendeur + visibilité dans le journal des ventes.
...
BEGIN
  ...
  WHERE c.gestionnaire_id = v_user_id
    AND c.gare_id = p_gare_depart_id
    AND c.statut = 'ouverte'
  ORDER BY c.opened_at DESC
  LIMIT 1;

  IF v_caisse_id IS NULL THEN
    RAISE EXCEPTION 'Ouvrez votre caisse a la gare de depart avant une vente cash';
  END IF;
...
DECLARE
  ...
  v_include_colis boolean := v_channel IS NULL OR v_channel = 'colis_autonome';
...
BEGIN
  ...
  FROM (
    ...
    SELECT
      ca.id AS booking_id,
      ca.created_at,
      UPPER(SUBSTRING(REPLACE(ca.id::text, '-', ''), 1, 8))::text AS reference,
      ...
      COALESCE(country.currency, 'XOF')::text AS currency,
      'colis_autonome'::text AS sale_channel,
      ca.statut_colis::text AS ticket_status,
      ca.vendeur_id AS seller_user_id,
      NULLIF(TRIM(u."firstName" || ' ' || u."lastName"), '')::text AS seller_name,
      (gd.name || ' -> ' || gdest.name)::text AS route_label,
