-- Stats colis : restriction par rôle (correctif migration 177).
-- Seuls owner / super_admin / comptable_compagnie voient les stats de toute
-- la compagnie et le filtre par agent. Tout autre rôle (vendeur,
-- vendeur_gare, chauffeur, contrôleur…) est FORCÉ sur sa propre activité :
-- p_vendeur_id est écrasé par son propre id côté serveur (le client peut
-- envoyer ce qu'il veut, la base ne renvoie que ses ventes), et la liste
-- des agents (filtre) est vide pour lui.
-- APPLIQUÉE EN PRODUCTION (apply_migration colis_stats_scope_par_role).
...
BEGIN
  ...
  IF NOT v_full_access THEN
    -- Rôle non privilégié : uniquement SA propre activité, quel que soit le
    -- filtre demandé par le client.
...
BEGIN
  ...
  -- Filtre "par agent" réservé aux rôles privilégiés : liste vide sinon
  -- (le client masque alors le filtre — de toute façon les stats sont déjà
  -- forcées sur sa propre activité côté serveur).
...
REVOKE EXECUTE ON FUNCTION public._colis_stats_full_access(uuid, uuid) FROM PUBLIC, anon;
