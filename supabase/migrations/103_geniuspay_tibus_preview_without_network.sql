-- 103_geniuspay_tibus_preview_without_network.sql
-- Page Tibus (payment/setup) : T = M × (1 + X% + Z%) + F  (sans Y réseau).
-- Y% est choisi et appliqué sur le checkout GeniusPay.
-- gatewayAmount = total Tibus ; GP ajuste T selon le réseau (T' = (V+F)/(1-Y-Z)).
...
DECLARE
  ...
  v_raw_network_min double precision;
  v_raw_network_max double precision;
  v_total double precision;
  v_total_network_min double precision;
  v_total_network_max double precision;
  v_configured text;
  v_fee_mode text;
...
BEGIN
  ...
  IF v_is_geniuspay THEN
    ...
    SELECT
      COALESCE(MIN(gpf."yPercent"), v_y),
      COALESCE(MAX(gpf."yPercent"), v_y)
    INTO v_y_min, v_y_max
    ...
    WHERE gpf."isActive" = true
      ...
      AND lower(gpf.network) NOT IN ('default', 'unknown');

    IF (v_y_min + v_z) >= 100 OR (v_y_max + v_z) >= 100 THEN
      RAISE EXCEPTION 'Taux GeniusPay invalides: Y+Z >= 100%%';
    END IF;

    v_raw := p_nominal_amount * (1 + (v_x + v_z) / 100.0) + v_f;
    v_total := CEIL(v_raw);
    v_fee_mode := 'on_top';

    v_raw_network_min := (v_v + v_f) / (1 - (v_y_min + v_z) / 100.0);
    v_raw_network_max := (v_v + v_f) / (1 - (v_y_max + v_z) / 100.0);
    v_total_network_min := CEIL(v_raw_network_min);
    v_total_network_max := CEIL(v_raw_network_max);
    ...
    RETURN jsonb_build_object(
      ...
      'gatewayFeePercentMin', v_y_min,
      'gatewayFeePercentMax', v_y_max,
      'geniusPayFeePercent', v_z,
      'fixedFee', v_f,
      'rawTotalAmount', v_raw,
      'totalAmount', v_total,
      'totalAmountMin', v_total_network_min,
      'totalAmountMax', v_total_network_max,
      'gatewayAmount', v_total,
      'feeMode', v_fee_mode,
      'feesDeferredToGateway', true,
      'networkFeeDeferred', true,
