...
// Copié du dépôt courrier_mobile (supabase/functions/_shared/fcm.ts) pour
// garder les mêmes conventions entre les deux projets. Nécessite le secret
// FCM_SERVICE_ACCOUNT (contenu JSON complet du fichier clé de compte de
// service, téléchargé depuis Firebase Console > Paramètres du projet >
// Comptes de service > Générer une nouvelle clé privée). À définir avec :
//
...
function getServiceAccount(): Record<string, unknown> {
  ...
  if (!raw) {
    throw new Error("Secret FCM_SERVICE_ACCOUNT manquant");
  }
