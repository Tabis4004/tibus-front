...
import { createGeniusPayCheckout } from "../_shared/geniuspay.ts";
import { countIssuedSeats, createAdminClient, resolveAppUserId } from "../_shared/issue-ticket.ts";

type PaymentGateway = "fedapay" | "geniuspay";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const activeGateway = await resolveActiveGateway(admin);
      ...
      if (message.includes("Configuration frais gateway")) {
        return jsonResponse({
          error: message.includes("Config existante")
            ? `Frais ${activeGateway} introuvables pour ce trajet. ${message}`
            : `Frais de paiement non configurés pour ce pays (gateway=${activeGateway}, mobile_money).`,
