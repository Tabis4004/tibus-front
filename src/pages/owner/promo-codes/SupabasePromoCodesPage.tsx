...
import { listOwnerRoutesSupabase } from "@/lib/supabase/owner-trips.ts";
import {
  createOwnerPromoCodeSupabase,
  deleteOwnerPromoCodeSupabase,
  listOwnerPromoCodesSupabase,
  updateOwnerPromoCodeSupabase,
  type OwnerPromoCode,
} from "@/lib/supabase/promo-codes.ts";
...
export default function SupabasePromoCodesPage() {
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      await createOwnerPromoCodeSupabase({
        code: newCode.trim(),
        discountType,
        ...
        trajetId: routeId !== "all" ? routeId : undefined,
      });
      ...
      toast.error(err instanceof Error ? err.message : t("promo.create_error", { defaultValue: "Erreur lors de la création" }));
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleToggle = async (promoId: string, currentActive: boolean) => {
    try {
      await updateOwnerPromoCodeSupabase(promoId, { isActive: !currentActive });
      toast.success(
        !currentActive
  ...
  const handleDelete = async (promoId: string) => {
    try {
      await deleteOwnerPromoCodeSupabase(promoId);
      ...
      {codes.length === 0 ? (
        ...
        <div className="grid gap-3">
          {codes.map((promo) => {
            ...
            const isExhausted = promo.maxUsage != null ? promo.usageCount >= promo.maxUsage : false;
                ...
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {t("promo.uses", { defaultValue: "utilisations" })}
                        </span>
                        {promo.routeLabel ? (
                          <span className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        onClick={() => void handleToggle(promo.id, promo.isActive)}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => void handleCreate()} disabled={creating} className="cursor-pointer">
