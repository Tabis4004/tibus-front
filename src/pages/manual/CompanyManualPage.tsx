...
export default function CompanyManualPage() {
  ...
  const locale = lng ?? "fr";
  const home = `/${locale}`;
  const ownerHome = `/${locale}/owner`;

  const canAccess =
    appUser.isReady && canAccessCompanyManual(appUser.roles, appUser.isSuperAdmin);
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <Badge variant="secondary" className="text-[10px] uppercase tracking-wide">
                {t("manual.company_badge", { defaultValue: "Owner & super administrateur" })}
              </Badge>
            </div>
