-- =============================================================================
-- Tibus 051 — Correctif vente guichet + réservation (idempotent)
-- Exécuter après 046–050 dans le SQL Editor Supabase
-- =============================================================================

-- 1) Statut caisse : autoriser en_reversement
...
-- 2) Fermer les doublons de caisses ouvertes (garde la plus récente par vendeur)
WITH ranked AS (
  SELECT c.id,
         ROW_NUMBER() OVER (
           PARTITION BY c.gestionnaire_id
           ORDER BY c.opened_at DESC NULLS LAST, c.id DESC
         ) AS rn
  FROM caisses_gares c
  WHERE c.statut IN ('ouverte', 'en_reversement')
)
UPDATE caisses_gares c
SET statut = 'cloturee', closed_at = COALESCE(c.closed_at, now())
FROM ranked r
...
BEGIN
  ...
  SELECT g.id INTO v_id FROM "Gares" g
  WHERE g."companyId" = p_company_id AND g.name = v_label LIMIT 1;
  IF v_id IS NOT NULL THEN RETURN v_id; END IF;
  INSERT INTO "Gares" (name, "companyId") VALUES (v_label, p_company_id) RETURNING id INTO v_id;
  RETURN v_id;
END;
$$;
...
BEGIN
  ...
  IF v_gare_id IS NULL THEN RAISE EXCEPTION 'Hub caisse compagnie introuvable'; END IF;
...
CREATE OR REPLACE FUNCTION public.get_seller_open_caisse_id(p_user_id uuid, p_company_id uuid)
RETURNS uuid
LANGUAGE sql
STABLE
SECURITY DEFINER
...
-- 4) Vente guichet : caisse session + verifyToken optionnel
