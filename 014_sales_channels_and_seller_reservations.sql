-- Canaux de vente/reservation et restriction voyageur.
-- A appliquer apres 011/012/013. Idempotent.
...
-- Un voyageur ne peut avoir qu'un seul billet paye sur le meme depart via le canal voyageur.
CREATE UNIQUE INDEX IF NOT EXISTS "reservationbus_one_traveler_ticket_per_departure"
  ON "ReservationBus" ("createdBy", "reservationId")
  WHERE COALESCE("saleChannel", 'traveler') = 'traveler';
...
BEGIN
  ...
  -- Vente directe seulement pour les roles rattaches a cette compagnie.
  IF NOT public.is_super_admin()
     AND NOT EXISTS (
       ...
       WHERE ur."userId" = v_user_id
         ...
         AND ro.name IN ('vendeur', 'owner')
     ) THEN
    RAISE EXCEPTION 'Vente directe reservee aux vendeurs de la compagnie';
  END IF;
